...
-- ¿Te animas a un último desafío?

-- ✏️Ejercicios de Integración JARDINERÍA

-- 1.- Calcula la fecha del primer y último pago realizado por cada uno de los clientes. El listado deberá mostrar el nombre y los apellidos de cada cliente.
...
-- 2.- Lista las ventas totales de los productos que hayan facturado más de 3000 euros.
...
-- 3.- Los clientes cuyo límite de crédito sea mayor que los pagos que haya realizado. (Sin utilizar INNER JOIN).
SELECT c.nombre_cliente, c.apellido_contacto, c.limite_credito,  
       (SELECT SUM(p.total) FROM pago p WHERE p.codigo_cliente = c.codigo_cliente) AS total_pagos
FROM cliente c
WHERE c.limite_credito > (SELECT SUM(p.total) FROM pago p WHERE p.codigo_cliente = c.codigo_cliente)

-- 4.- Devuelve el nombre, los apellidos y el correo electrónico de los empleados que están a cargo de Alberto Soria.
...
-- 5.- Devuelve un listado que muestre solamente los clientes que sí han realizado ningún pago. (Recomendamos resolver este ejercicio con Subconsultas del tipo IN y NOT IN).
SELECT c.nombre_cliente, c.apellido_contacto
FROM cliente c
WHERE c.codigo_cliente NOT IN (SELECT p.codigo_cliente FROM pago p)

-- 6.- Devuelve un listado de los productos (código y nombre) que han aparecido en un pedido alguna vez. (Recomendamos resolver este ejercicio con Subconsultas del tipo EXISTS y NOT EXISTS).
SELECT p.codigo_producto, p.nombre
FROM producto p
WHERE EXISTS (SELECT 1 FROM detalle_pedido dp WHERE dp.codigo_producto = p.codigo_producto)

-- 7.- Devuelve el nombre del producto que tiene el precio de venta más caro. (Recomendamos resolver este ejercicio con Subconsultas del tipo ALL y ANY).
SELECT p.nombre
FROM producto p
WHERE p.precio_venta = (SELECT MAX(precio_venta) FROM producto)

-- 8.- Devuelve el nombre del producto que tenga el precio de venta más caro (puedes realizarlo ordenando la tabla).
SELECT p.nombre
FROM producto p
...
SELECT SUM(dp.cantidad) AS total_cantidad
...
-- 11.- Devuelve el producto que más unidades tiene en stock.
SELECT p.nombre, p.cantidad_en_stock
FROM producto p
ORDER BY p.cantidad_en_stock DESC
LIMIT 1

-- 12.- Devuelve un listado que muestre solamente los clientes que no han realizado ningún pago. (Recomendamos resolver este ejercicio con Subconsultas del tipo ALL y ANY).
SELECT c.nombre_cliente, c.apellido_contacto
FROM cliente c
WHERE c.codigo_cliente NOT IN (SELECT p.codigo_cliente FROM pago p)

-- 13.- Devuelve un listado de los productos que nunca han aparecido en un pedido. (Recomendamos resolver este ejercicio con Subconsultas del tipo EXISTE y NOT EXISTS).
SELECT p.codigo_producto, p.nombre
FROM producto p
WHERE NOT EXISTS (SELECT 1 FROM detalle_pedido dp WHERE dp.codigo_producto = p.codigo_producto)

-- 14.- Calcula el número de productos diferentes que hay en cada uno de los pedidos. 
SELECT p.codigo_pedido, COUNT(DISTINCT dp.codigo_producto) AS num_productos
FROM pedido p
JOIN detalle_pedido dp ON p.codigo_pedido = dp.codigo_pedido
GROUP BY p.codigo_pedido

-- 15.- Devuelve el nombre de los representantes de ventas y el número de clientes al que atiende cada uno.
...
SELECT c.ciudad, COUNT(c.codigo_cliente) AS num_clientes
FROM cliente c
WHERE c.ciudad LIKE 'M%'
GROUP BY c.ciudad

-- 17.- Calcule el número de clientes que no tiene asignado representante de ventas.
SELECT COUNT(codigo_cliente) AS num_clientes
FROM cliente
WHERE codigo_empleado_rep_ventas NOT IN (SELECT codigo_empleado FROM empleado)

-- 18.- Devuelve un listado de los 20 productos más vendidos y el número total de unidades que se han vendido de cada uno. El listado deberá estar ordenado por el número total de unidades vendidas descendentes.
